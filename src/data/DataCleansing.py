...
CSV_PATH = os.path.join(INTERIM_DIR, "post_eda.csv")
LOCAL_OUTPUT_DIR = os.path.join(EDA_OUTPUT_DIR, "cleansing")
OUTPUT_DIR_SPLIT = INTERIM_DIR
ensure_dirs(LOCAL_OUTPUT_DIR)

# EDA image subfolders
HISTS_DIR = os.path.join(LOCAL_OUTPUT_DIR, "histograms_numeric")
BOXES_DIR = os.path.join(LOCAL_OUTPUT_DIR, "boxplots_numeric")
BARS_DIR = os.path.join(LOCAL_OUTPUT_DIR, "barplots_categorical")
for _subdir in (HISTS_DIR, BOXES_DIR, BARS_DIR):
    ensure_dirs(_subdir)

# EDA tables main Excel
EDA_XLSX_NAME = "analysis_output.xlsx"
EDA_XLSX_PATH = os.path.join(LOCAL_OUTPUT_DIR, EDA_XLSX_NAME)

# Image outputs
IMG_HEAT_SPEARMAN = os.path.join(LOCAL_OUTPUT_DIR, "spearman_heatmap.png")
IMG_HEAT_CRAMERS = os.path.join(LOCAL_OUTPUT_DIR, "categorical_cramersV_heatmap.png")
IMG_ALL_NUM_BOX = os.path.join(LOCAL_OUTPUT_DIR, "boxplot_all_numeric_grid.png")
IMG_PAIRPLOT_NUM = os.path.join(LOCAL_OUTPUT_DIR, "pairplot_numeric_grid.png")

# Statistical results
TESTS_XLSX = os.path.join(LOCAL_OUTPUT_DIR, "test_results.xlsx")
TESTS_CSV_SUMMARY = os.path.join(LOCAL_OUTPUT_DIR, "test_results_summary.csv")
TESTS_CSV_SIGNIF = os.path.join(LOCAL_OUTPUT_DIR, "test_results_significant.csv")

# Full post-cleaning CSV
POST_CLEAN_CSV = os.path.join(LOCAL_OUTPUT_DIR, "post_cleaning.csv")

# Split outputs
LOCAL_SPLIT_DIR = os.path.join(OUTPUT_DIR_SPLIT, "split")
ensure_dirs(LOCAL_SPLIT_DIR)
TRAIN_OUT_PATH = os.path.join(LOCAL_SPLIT_DIR, "train.csv")
VAL_OUT_PATH = os.path.join(LOCAL_SPLIT_DIR, "val.csv")
TEST_OUT_PATH = os.path.join(LOCAL_SPLIT_DIR, "test.csv")
...
def write_eda_excel_safely(
    ...
    """Write EDA tables to Excel safely."""
    ...
    try:
        _do_write(xlsx_output_path)
        print_ok(f"EDA Excel saved to: {xlsx_output_path}")
    except PermissionError:
        alt = _ts_copy_path(LOCAL_OUTPUT_DIR, stem=os.path.splitext(os.path.basename(xlsx_output_path))[0])
        _do_write(alt)
        print_warn(f"Excel was locked. Wrote a timestamped copy: {alt}")
    except (OSError, ValueError) as e_xlsx:
        alt = _ts_copy_path(LOCAL_OUTPUT_DIR, stem="analysis_output_fallback")
        try:
            _do_write(alt)
            print_warn(f"Excel write issue ({e_xlsx}). Wrote fallback: {alt}")
        except (IOError, OSError, PermissionError) as e2:
            print_error(f"Failed to write Excel outputs: {e2}")


def print_bin_dist(title_name: str, series_in: pd.Series) -> None:
    """Print binary distribution summary."""
...
def save_multi_boxplot_grid(df_num_grid: pd.DataFrame, out_file_path: str, ncols: int = 4) -> None:
    ...
    if not SEABORN_OK:
        print_warn("seaborn not available - skipping boxplot grid")
        return
    ...
    for idx, varname in enumerate(cols_present):
        ...
        try:
            ...
            ax.set_title(str(varname), fontsize=10)
            ax.set_xlabel("")
            ax.set_ylabel("")
...
def cramers_v_corrected(x_series: pd.Series, y_series: pd.Series) -> float:
    ...
    if n_observations == 0:
        return float("nan")
    phi2 = float(chi2_stat) / n_observations
    r_dim, k_dim = crosstab.shape
    if n_observations > 1:
...
def main() -> None:
    ...
    print_ok(f"Data loaded: {CSV_PATH}")
    ...
    if cols_found:
        ...
        print_info(f"Dropped columns: {cols_found}")
    ...
    if obj_cols:
        try:
            ...
            print_warn(f"Could not convert object columns to category: {e}")
    ...
    if "Visibility(mi)" in df.columns:
        ...
        vis_cat[vis_vals <= 3.0] = "Low"
        vis_cat[vis_vals.isna()] = "Missing"
    ...
    for b in [v.strip() for v in _raw_bool_vars]:
        if b in df.columns:
            try:
                ...
                try:
                    df[b] = pd.Series(df[b]).astype("boolean")
                except (TypeError, ValueError):
                    print_warn(f"Could not convert {b} to boolean: {e}")
    ...
    if "Airport_Code" in df.columns:
        ...
        if len(csum[csum >= 0.5]) > 0:
            ...
            print_info(f"Filled Airport_Code with median category: {median_cat}")
    ...
    if "FARS__ALC_RES" in df.columns:
        ...
        print_info("ALC_category created and 'FARS__ALC_RES' dropped.")

    print_ok("Cleaning & feature-engineering complete.")
    ...
    print_ok("Added 'Severity_2cat' categorical variable.")
    ...
    if INCLUDE_BOOLS_IN_SPEARMAN and bool_cols:
        for bcol in bool_cols:
            df_corr_input[bcol] = df[bcol].astype("Int64")
    if SEABORN_OK and not df_corr_input.empty and df_corr_input.shape[1] >= 2:
        try:
            ...
            print_ok(f"Spearman heatmap saved: {IMG_HEAT_SPEARMAN}")
        except (ValueError, TypeError, RuntimeError) as e:
            print_warn(f"Spearman heatmap skipped: {e}")

    # Numeric histograms & boxplots
    if SEABORN_OK:
        for nc in num_cols:
            s_num = pd.to_numeric(df[nc], errors="coerce").dropna()
            p_hist = os.path.join(HISTS_DIR, f"{safe_name(nc)}_hist.png")
            if s_num.empty:
                ...
                try:
                    ...
                    print_warn(f"Histogram skipped for {nc}: {e}")
            p_box = os.path.join(BOXES_DIR, f"{safe_name(nc)}_box.png")
            if s_num.empty:
                ...
                try:
                    ...
                    print_warn(f"Boxplot skipped for {nc}: {e}")
    ...
    if num_cols:
        ...
        if not df_num_for_grid.empty:
            ...
            print_ok(f"Boxplot grid saved: {IMG_ALL_NUM_BOX}")
    ...
    for var in cat_bool_cols:
        ...
        if total_counts > 0:
            ...
            if len(perc) > TOP_K:
                ...
                other_val = float(perc.iloc[TOP_K - 1:].sum())
                ...
                try:
                    ...
                    for patch in ax_bar.patches:
                        if isinstance(patch, Rectangle):
                            ...
                            if np.isfinite(w):
                                ax_bar.text(w + 0.5, y, f"{w:.1f}%", va="center")
                    ensure_fig_saved_close(p_bar, dpi=150)
                except (ValueError, TypeError, RuntimeError) as e:
                    print_warn(f"Barplot skipped for {var}: {e}")
        else:
            fig = plt.figure(figsize=(8, 3))
            plt.title(f"{var} - (no data)")
            plt.axis("off")
            fig.savefig(p_bar, dpi=140, bbox_inches="tight")
            plt.close(fig)
    ...
    # Categorical heatmap (Cramer's V)
    cramers_matrix: Optional[pd.DataFrame] = None
    if SCIPY_OK and SEABORN_OK and len(cat_bool_cols) >= 2:
        ...
        if len(cols_for_heat) >= 2:
            ...
            for i, ci in enumerate(cols_for_heat):
                for j, cj in enumerate(cols_for_heat):
                    if j < i:
                        continue
                    v = 1.0 if ci == cj else cramers_v_corrected(df[ci], df[cj])
                    cramers_matrix.loc[ci, cj] = v
                    cramers_matrix.loc[cj, ci] = v
            ...
            try:
                ...
                print_ok(f"Cramer's V heatmap saved: {IMG_HEAT_CRAMERS}")
            except (ValueError, TypeError, RuntimeError) as e:
                print_warn(f"Cramer's V heatmap skipped: {e}")
        ...
        if not df_num_pp.empty:
            ...
            if not df_num_pp.empty:
                ...
                try:
                    ...
                    print_ok(f"Pairplot saved: {IMG_PAIRPLOT_NUM}")
                except (ValueError, TypeError, RuntimeError, MemoryError) as e:
                    print_warn(f"Pairplot skipped: {e}")
    ...
    print(
        f"\nOutputs directory: {LOCAL_OUTPUT_DIR}\n"
        f" - Excel (tables): {EDA_XLSX_PATH}\n"
        f" - Spearman PNG : {IMG_HEAT_SPEARMAN}\n"
        f" - Cramer's PNG : {IMG_HEAT_CRAMERS}\n"
        f" - Boxplot grid : {IMG_ALL_NUM_BOX}\n"
        f" - Pairplot PNG : {IMG_PAIRPLOT_NUM}\n"
        f" - Hist dir : {HISTS_DIR}\n"
        f" - Box dir : {BOXES_DIR}\n"
        f" - Bar dir : {BARS_DIR}\n"
    )
    ...
    if not SCIPY_OK:
        print_warn("Skipping statistical tests - SciPy not available")
    else:
        ...
        for col in [c for c in df.columns if c not in {TARGET_COL} and df[c].notna().any()]:
            ...
            try:
                if is_numeric_dtype(s_pred):
                    if y_target.nunique(dropna=True) == 2:
                        y_groups = y_target.dropna().unique().tolist()
                        g1 = s_pred[y_target == y_groups[0]].dropna()
                        g2 = s_pred[y_target == y_groups[1]].dropna()
                ...
                elif isinstance(s_pred.dtype, CategoricalDtype) or is_object_dtype(s_pred) or is_string_dtype(s_pred) or is_bool_dtype(s_pred):
                    cont = pd.crosstab(s_pred, y_target)
                    r, k = cont.shape
                    if r == 2 and k == 2:
                        odds, p_val = fisher_exact(cont)
                        row.update({"test": "Fisher exact (2x2)", "n": int(cont.values.sum()), "stat": float(odds), "p_value": float(p_val)})
                    else:
                        chi2, p_val, dof, expected = chi2_contingency(cont)
                        n_obs = int(cont.values.sum())
                        denom_local = max(1.0, min(r - 1, k - 1))
        ...
        try:
            ...
            tests_df.to_csv(TESTS_CSV_SUMMARY, index=False)
            signif_df.to_csv(TESTS_CSV_SIGNIF, index=False)
            print_ok("Saved updated statistical test results:")
            print(f" - Excel full: {TESTS_XLSX}")
            print(f" - CSV summary: {TESTS_CSV_SUMMARY}")
            print(f" - CSV significant: {TESTS_CSV_SIGNIF}")
        except Exception as e:
            print_warn(f"Failed to save updated statistical results: {e}")

    # 6) Export full post-cleaning dataframe
    try:
        df.to_csv(POST_CLEAN_CSV, index=False, encoding="utf-8")
        print_ok(f"Full cleaned dataframe saved to: {POST_CLEAN_CSV}")
    except (IOError, OSError, PermissionError) as e:
        print_warn(f"Failed to save post-cleaning CSV: {e}")

    # 7) Create Severity_bin and stratified split
    print_info("Creating binary target and splitting data...")
    if "Severity" not in df.columns:
    ...
    df_split = df.copy()
    df_split["Severity_bin"] = severity_bin
    ...
    print_ok("Severity_bin created: 0=[1,2], 1=[3,4]")
    if n_removed > 0:
        print_warn(f"Removed {n_removed:,} rows with invalid Severity")
    ...
    try:
        ...
        print_ok("Saved splits (stratified by Severity_bin):")
        print(" - Train:", TRAIN_OUT_PATH)
        print(" - Val :", VAL_OUT_PATH)
        print(" - Test :", TEST_OUT_PATH)
    except (IOError, OSError, PermissionError) as e:
        print_error(f"Failed to write splits: {e}")
    ...
    print_ok("Done: cleaning, EDA, stats, and split complete.")
