...
CSV_PATH = os.path.join(INTERIM_DIR, "df_prepared.csv")
EDA_DIR = EDA_OUTPUT_DIR
ensure_dirs(EDA_DIR)
...
def safe_write_eda_excel(
    ...
    try:
        ...
        print_ok(f"File saved to: {output_path}")
        ...
        print_warn("Original file was locked (likely open in Excel).")
        print_ok(f"Wrote timestamped copy: {alt_path}")
    except (OSError, ValueError) as err:
        print_error(f"Failed to write Excel: {err}")
...
def save_multi_boxplot_grid(df_num_grid: pd.DataFrame, out_file_path: str, ncols: int = 4) -> None:
    ...
    if not cols_present:
        print_warn("No valid numeric columns for boxplot grid")
        return
    ...
    for idx, varname in enumerate(cols_present):
        ...
        try:
            ...
            print_warn(f"Boxplot failed for {varname}: {err}")
            ax_current.axis("off")
    ...
    plt.close(fig_grid)
    print_ok(f"Saved boxplot grid: {out_file_path}")
...
def main() -> None:
    ...
    print_ok("Data loaded successfully.")
    ...
    if (not df_corr_input.empty) and (df_corr_input.shape[1] >= 2) and SEABORN_OK:
        try:
            ...
            print_ok(f"Saved: {IMG_HEAT_SPEARMAN}")
        except (ValueError, RuntimeError) as e:
            print_warn(f"Spearman heatmap skipped: {e}")
    else:
        print_warn("Insufficient columns for Spearman heatmap")

    # Individual numeric plots
    if SEABORN_OK:
        print_info("Generating individual numeric plots...")
        for nc in num_cols:
            s_num = pd.to_numeric(df[nc], errors="coerce").dropna()

            # Histogram
            hist_path = os.path.join(HISTS_DIR, f"{safe_name(nc)}_hist.png")
            if s_num.empty:
                ...
                try:
                    ...
                    print_warn(f"Histogram skipped for {nc}: {hist_err}")

            # Boxplot
            box_path = os.path.join(BOXES_DIR, f"{safe_name(nc)}_box.png")
            if s_num.empty:
                ...
                try:
                    ...
                    print_warn(f"Boxplot skipped for {nc}: {box_err}")

    # Grid of all numeric boxplots
    if num_cols:
        print_info("Generating boxplot grid...")
        save_multi_boxplot_grid(df[num_cols], IMG_ALL_NUM_BOX, ncols=4)

    # Categorical barplots + tables
    print_info("Generating categorical barplots...")
    ...
    for var in cat_bool_cols:
        ...
        img_path = os.path.join(BARS_DIR, f"{safe_name(var)}_bar.png")
        if total_counts == 0:
            fig = plt.figure(figsize=(8, 3))
        ...
        if SEABORN_OK:
            try:
                ...
                print_warn(f"Barplot skipped for {var}: {bar_err}")
    ...
    if SCIPY_OK and SEABORN_OK and len(cat_bool_cols) >= 2:
        ...
        if len(cols_for_heat) >= 2:
            print_info("Computing Cramer's V heatmap...")
            ...
            try:
                ...
                print_ok(f"Saved: {IMG_HEAT_CRAMERS}")
            except (ValueError, RuntimeError) as e:
    ...
    if {"Start_Lat", "Start_Lng"}.issubset(df.columns):
        print_info("Generating hexbin maps...")
        ...
        if valid.any():
            ...
            try:
                ...
                print_ok(f"Saved: {IMG_HEXBIN_PLAIN}")
            except (ValueError, RuntimeError) as e:
                print_warn(f"Hexbin (plain) skipped: {e}")
            ...
            try:
                ...
                print_ok(f"Saved: {IMG_HEXBIN_LABELS}")
            except (ValueError, RuntimeError) as e:
                print_warn(f"Labeled hexbin skipped: {e}")
    ...
    if num_cols and SEABORN_OK:
        print_info("Generating pairplot...")
        ...
        if not df_num_pp.empty:
            ...
            if not df_num_pp.empty:
                try:
                    ...
                    plt.close(pg.figure)
                    print_ok(f"Saved: {IMG_PAIRPLOT_NUM}")
                except (ValueError, RuntimeError) as e:
                    print_warn(f"Pairplot skipped: {e}")

    # Write EDA Excel
    print_info("Writing EDA summary to Excel...")
    ...
    print(
        ...
        f"   - Cramer's V heatmap: {IMG_HEAT_CRAMERS}\n"
    ...
    if TEXTBLOB_OK and {"Description", "Severity"}.issubset(df.columns):
        print_info("Performing sentiment analysis...")
        ...
        try:
            ...
            print_warn(f"TextBlob sentiment skipped: {e}")
        ...
        try:
            ...
            print_ok(f"Saved sentiment Excel: {sentiment_xlsx}")
        except PermissionError:
            alt = os.path.join(
                sentiment_out_dir,
                f"severity_sentiment_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            )
            with pd.ExcelWriter(alt, engine="xlsxwriter") as writer:
                ...
                overall.to_excel(writer, sheet_name="overall_counts", index=False)
            print_warn(f"Excel locked. Saved timestamped copy: {alt}")
        except (OSError, ValueError) as e:
            print_error(f"Failed to write sentiment Excel: {e}")
        ...
        try:
            ...
            print_ok(f"Saved: {bar_path}")
        except (ValueError, RuntimeError) as e:
            print_warn(f"Sentiment bar plot skipped: {e}")
        ...
        try:
            ...
            print_ok(f"Saved: {scatter_path}")
        except (ValueError, RuntimeError) as e:
            print_warn(f"Sentiment scatter plot skipped: {e}")
    else:
        if not TEXTBLOB_OK:
            print_warn("Skipping sentiment analysis: TextBlob not available")
        else:
            print_warn("Skipping sentiment analysis: 'Description' or 'Severity' columns missing")

    print_ok("EDA complete! (No train/val/test split performed.)")

    # Export full EDA dataframe
    POST_EDA_CSV_PATH = os.path.join(INTERIM_DIR, "post_eda.csv")

    try:
        df.to_csv(POST_EDA_CSV_PATH, index=False, encoding="utf-8")
        print_ok(f"Full EDA dataframe saved successfully to:\n{POST_EDA_CSV_PATH}")
