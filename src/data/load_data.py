# ==========================================================
# FARS Data Loading and Merging with NYC Accidents
# ==========================================================
from src.utils.helpers import *

# File-specific paths
BASE = Path(os.path.join(PROJECT_ROOT, "src", "data", "raw", "interim"))
BASE_FARS = Path(os.path.join(PROJECT_ROOT, "src", "data", "raw", "interim", "fars_data"))
OUTPUTS = Path(os.path.join(PROJECT_ROOT, "src", "data", "raw", "interim", "output_data"))

US_ACCIDENTS_PKL = BASE / "us_accidents.pkl"
BORO_CSV = BASE / "NYC_Zipcodes_NYC_Zip_2020.csv"
FARS_DATA_DIR = BASE_FARS / "fars_data"
LOCAL_INTERIM_DIR = OUTPUTS

FARS_SLIM_PARQUET = LOCAL_INTERIM_DIR / "fars_nyc_slim.parquet"
FARS_SLIM_CSV = LOCAL_INTERIM_DIR / "fars_nyc_slim.csv"
NYC_ACC_OUT = LOCAL_INTERIM_DIR / "NYC_Accidents_with_FARS_raw.csv"
...
def _download_fars_zip(year: int) -> Optional[Path]:
    """Download FARS zip file if not present locally."""
    if not REQUESTS_OK or not TQDM_OK:
        print_warn("requests or tqdm not available - cannot download FARS data")
        return None
    ...
    for u in FARS_URLS:
        ...
        try:
            ...
            print_warn(f"Download failed {url}: {e}")
            time.sleep(1)
    return None


def _read_member_from_zip(zip_path: Path, name_regex: str) -> pd.DataFrame:
    """Read a CSV member from a zip file."""
...
def _load_fars_year(year: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Load FARS data for a specific year."""
...
def build_fars_nyc_slim(years: List[int]) -> pd.DataFrame:
    """Build a slim FARS dataset for NYC."""
    ...
    if "LATITUDE" in acc_s.columns:
        acc_s = acc_s.rename(columns={"LATITUDE": "FARS_LAT"})
    if "LATITUD" in acc_s.columns:
        acc_s = acc_s.rename(columns={"LATITUD": "FARS_LAT"})
    if "LONGITUD" in acc_s.columns:
        acc_s = acc_s.rename(columns={"LONGITUD": "FARS_LON"})
    if "LONGITUDE" in acc_s.columns:
        acc_s = acc_s.rename(columns={"LONGITUDE": "FARS_LON"})
    ...
    if "AGE" in per_s.columns:
        per_s = per_s.rename(columns={"AGE": "DRIVER_AGE"})
    elif "AGE_IM" in per_s.columns:
        per_s = per_s.rename(columns={"AGE_IM": "DRIVER_AGE"})
...
def build_nyc_accidents_with_borough() -> pd.DataFrame:
    """First merge: accidents with boroughs (ZIP)."""
    ...
    print_info("Accidents (NYC-filter, pre-borough-merge):")
    df.info()

    print_info("Borough lookup (cleaned):")
    df_boros.info()
    ...
    print_info("Accidents + Boroughs (after first merge):")
    df.info()
    return df
...
def spatial_join_nearest_borough_cap(left: pd.DataFrame, fars_slim: pd.DataFrame) -> pd.DataFrame:
    ...
    if not GEOPANDAS_OK:
        print_warn("geopandas not available - cannot perform spatial join")
        return left
    ...
    print_info("FARS slim (NYC-only, geo kept):")
    r.info()
    ...
    print_info("Accidents + Boroughs + FARS (after second merge):")
    out.info()
    return out.reset_index(drop=True)


def main() -> None:
    # 1) Accidents with Boroughs
    ...
    if FARS_SLIM_PARQUET.exists() or FARS_SLIM_CSV.exists():
        ...
        print_info(f"Saved FARS slim to: {slim_path}")
    ...
    print_ok(f"Final saved to: {NYC_ACC_OUT}")
