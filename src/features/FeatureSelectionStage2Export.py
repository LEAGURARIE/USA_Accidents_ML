...
TRAIN_FE_PATH = os.path.join(FE_OUTPUT_DIR, "train_fe.csv")
VALID_FE_PATH = os.path.join(FE_OUTPUT_DIR, "valid_fe.csv")
TEST_FE_PATH = os.path.join(FE_OUTPUT_DIR, "test_fe.csv")
STAGE2_IDX_CSV = os.path.join(FE_OUTPUT_DIR, "stage2_selected_feature_indices.csv")
...
def resolve_target_name(df: pd.DataFrame) -> str:
    ...
    print_info(f"Using target column: {resolved}")
    return resolved
...
def build_preprocess_from_train(
    ...
    """
    Builds and fits the ColumnTransformer on TRAIN only.
    """
    ...
    ordinal_pipe = build_ordinal_pipeline(ordinal_cols) if ordinal_cols else "drop"
    ...
    print_info(f"Fitted ColumnTransformer on TRAIN: {x_train_t.shape[1]} transformed features")
...
def export_processed_split(
    ...
    try:
        ...
        print_warn(f"Could not write Parquet for {split_name}: {ex}")

    print_ok(f"Saved {split_name} to:")
    print(f"     {csv_path}")
...
def export_target_vector(
    ...
    print_ok(f"Saved {split_name} target to:")
    print(f"     {csv_path}")
...
def main() -> None:
    ...
    train_df = load_csv_safe(TRAIN_FE_PATH)
    valid_df = load_csv_safe(VALID_FE_PATH)
    test_df = load_csv_safe(TEST_FE_PATH)

    print_info("Loaded FE splits:")
    ...
    print_info(f"Ordinal columns (by name/map): {ordinal_cols}")
    ...
    print_info(f"Numeric cols: {len(numeric_cols)}")
    print_info(f"Ordinal cols: {len(ordinal_cols_final)} | Categorical cols: {len(categorical_cols)}")
    ...
    stage2_idx_df = load_csv_safe(STAGE2_IDX_CSV)
    ...
    print_info(f"Stage-2 selected feature count: {stage2_indices.size}")
    ...
    print_info(f"After Stage-2 selection (columns): {x_train_sel.shape[1]}")
    ...
    print_ok("Done. Stage-2 processed datasets are ready.")
