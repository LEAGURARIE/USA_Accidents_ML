...
def main() -> None:
    ...
    # 2) Drop any target-leakage columns (keep exactly TARGET)
    leak_cols = [c for c in df.columns if c.startswith("Severity") and c != TARGET]
    if leak_cols:
        print_info(f"Dropping leakage columns: {leak_cols}")
    ...
    # 8) Detect binary-like floats (values in {0,1}) that are not integers
    binary_like = []
    ...
    print_info(f"Columns found & will be dropped: {existing_to_drop}")
    if existing_to_drop:
    ...
    if binary_like:
        ...
        if castable:
            print_info(f"Casting binary-like float -> Int64 for: {castable}")
