...
DROP_AFTER_IMPUTE: List[str] = [
    ...
    "Severity", "Severity_bin", "Distance(mi)", "Precipitation(in)",
    "Weather_ConditionGroup", "Distance_group", "Visibility_2cat",
    "FARS__SEX", "FARS__SEX_label", "FARS__BODY_TYP",
]

SEED = RANDOM_STATE
RNG = np.random.default_rng(SEED)

ensure_dirs(ARTIFACT_DIR, FE_OUTPUT_DIR)
...
def load_dataframe_any(path: str) -> pd.DataFrame:
    ...
    if os.path.isdir(path):
        ...
        df_loaded = load_csv_safe(csv_candidates[0])
    else:
        if not path.lower().endswith(".csv"):
            raise ValueError(f"Expected .csv, got: {path}")
        df_loaded = load_csv_safe(path)
...
def export_dataset(df: pd.DataFrame, stem: str, tag: str) -> None:
    info_preview(df, tag)
    csv_path = os.path.join(FE_OUTPUT_DIR, f"{stem}.csv")
    pq_path = os.path.join(FE_OUTPUT_DIR, f"{stem}.parquet")
    df.to_csv(csv_path, index=False)
    try:
        df.to_parquet(pq_path, index=False)
    except (ImportError, ValueError, TypeError, OSError) as e:
        print_warn(f"Parquet not written ({e}). CSV still saved.")
...
def apply_core_fe_inplace(df_work: pd.DataFrame, fe_levels: Dict) -> None:
    ...
    df_work["Is_Weekend"] = df_work["Event_DOW"].isin([4, 5])
...
if __name__ == "__main__":
    ...
    if len(used_knn_cols_fit) > 0:
        ...
        print_warn("KNNImputer skipped: no KNN columns available in TRAIN.")
    ...
    if len(used_mice_cols_fit) >= 2:
        ...
        print_warn(f"IterativeImputer (MICE) skipped: need >=2 cols, have {len(used_mice_cols_fit)} - {used_mice_cols_fit}")
    ...
    print_ok(f"Done. Artifacts: {ARTIFACT_DIR}")
    print_ok(f"Outputs: {FE_OUTPUT_DIR}")
